...
class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations, n_keys, pinns_params=None, vel_start_time=0.0) -> None:
        ...
        self.deform = DeformModel(max_time=dataset.max_time, vel_start_time=vel_start_time, light=dataset.light, physics_code=dataset.physics_code, use_affine=True)
    ...
    def train_step(self):
        ...
        if self.iteration < self.opt.warm_up:
            ...
            d_xyz, d_rotation, d_scaling = self.deform.step_no_rot(self.gaussians.get_xyz.detach(), time_input,
        ...
        if self.pinns_computer is not None and self.iteration >= self.opt.warm_up:
            ...
            # 关键修复：使用try-except捕获PINNs计算中的异常
            try:
                if self.iteration == self.opt.warm_up:
        ...
        with torch.no_grad():
            ...
            loss_value = loss.item()  # 保存loss值用于日志
    ...
    def test_step(self):
        ...
        out = render(viewpoint_camera=cur_cam, gaussians=self.gaussians, pipe=self.pipe, bg_color=self.background, d_xyz=d_xyz,
